...
<script>
	$(function(){
		$("#wxOauthList").bootstrapTable({
			url:"${managerPath}/weixin/wxOauth/list.do",
	    	...
	    	columns: [{ checkbox: true},{
	        	...
	        	formatter:function(value,row,index) {
	        		var url = "${managerPath}/weixin/wxOauth/form.do?oauthId="+row.oauthId;
	...
	$("#addOauthBtn").click(function(){
		location.href ="${managerPath}/weixin/wxOauth/form.do"; 
	})
	
	//删除按钮
	$("#delOauthBtn").click(function(){
		//获取checkbox选中的数据
		var rows = $("#wxOauthList").bootstrapTable("getSelections");
		//没有选中checkbox
		if(rows.length <= 0){
	...
	$("#deleteOauthBtn").click(function(){
		...
		$.ajax({
			type: "post",
			url: "${managerPath}/weixin/wxOauth/delete.do",
	...
	function search(){
        ...
        var params = $('#wxOauthList').bootstrapTable('getOptions');
        params.queryParams = function(params) {  
        	$.extend(params,search);
	        return params;  
       	}  
   	 	$("#wxOauthList").bootstrapTable('refresh', {query:$("form[name='searchForm']").serializeJSON()});
	}
</script>
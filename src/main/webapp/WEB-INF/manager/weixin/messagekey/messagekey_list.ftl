...
<script>
			...
			columns: [{checkbox:'true'},
			{
			    ...
			    formatter:function(value,row,index){
			    	if(row.newsEntity.newsType == 0){
			    		return "<a style='cursor: pointer;' id='keyMessageIds' class='editMessage' data-original-title='修改' data-id=" + row.passiveMessageId + ">" + row.newsEntity.newsMasterArticle.basicTitle + "</a>";
			    	}
			    	else if(row.newsEntity.newsType == 1){
			    		
			    			return "<a style='cursor: pointer;' id='keyMessageIds' class='editMessage' data-original-title='修改' data-id=" + row.passiveMessageId + ">主图文标题：" + row.newsEntity.newsMasterArticle.basicTitle + "子图文标题：" +row.newsEntity.childs.basicTitle + "</a>";
			    		
			    	}
			    	else if(row.newsEntity.newsType == 2){
			    		return "<a style='cursor: pointer;' id='keyMessageIds' class='editMessage'   data-original-title='修改' data-id=" + row.passiveMessageId + ">" + row.newsEntity.newsContent + "</a>";
			    	}
	...
	$("#deleteMessagekey").click(function(){
		...
		var keyMessageIds = [];
		keyMessageIds = $("#keyMessageIds").attr("data-id");
		$.ajax({		
		    type:"GET",
